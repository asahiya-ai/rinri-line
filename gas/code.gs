...
function handleSummarize(data) {
  ...
  var userMessage = '以下のモーニングセミナー文字起こしを' + speakerCount + '人分の要約にまとめてください。\n\n'
    + '【ルール（必ず守ること）】\n'
    + '・ヘッダーは必ず【講話者】と書く（名前・肩書きは一切使わない）\n'
    + '・各要約の冒頭にリンちゃんらしい一言コメントを入れる\n'
    + '  （例：「コケコッコー！今週もすてきなお話だったよ🐔✨」）\n'
    + '・個人的エピソード（借金・倒産・病気・死・自殺・離婚・家族の不幸など）は\n'
    + '  絶対に具体的に書かない。必ず「深い闇を乗り越えた経験から」\n'
    + '  「言葉にできないほどの試練を経て」などの抽象的表現に置き換える\n'
    + '・読んだ人が「どんな話だったんだろう？」と気になるような書き方にする\n'
    + '  具体的な出来事は書かず、その人の変化・学び・メッセージだけを伝える\n'
    + '・講話の核心・名言はあえて書き切らず「続きはぜひセミナーで！」的な余韻を残す\n'
    + '・末尾に「今週もセミナーで一緒に学ぼう！」のような来週への期待感を添える\n'
    + '・1ブロック300〜400文字（冒頭コメント＋【講話者】本文＋末尾期待感を含む）\n'
    + '・ヘッダー【講話者】の直後は必ず改行する\n'
    + '・参加者数は記載しない\n\n'
    + '文字起こし:\n' + transcription + '\n\n'
    + 'JSON形式のみで回答してください：\n'
    + '{"summaries": ["1人目の要約テキスト", "2人目の要約テキスト"]}';
